...
function customAssign(...objects) {
  ...
  if((Object.keys(objects).length) < 2) return "Помилка: Має бути передано принаймні два об'єкти.";
...
function customEntries(obj) {
  ...
  if(typeof(obj) !== "object") return "Помилка: Аргумент не є об'єктом.";
  let now = (Object.entries(obj));
    return(now);
}
...
function customObjectFromEntries(entries) {
  ...
  if(!(Array.isArray(entries))) return "Помилка: Вхідний аргумент має бути масивом.";
  let arr = entries.map(([key, value]) => {
	if (key === "number") {
		return key.toString();
	} else {
		return [key, value];
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if(typeof(obj1, obj2) !== "object") return null;
  let obj = Object.assign(obj1, obj2);
  let freez = Object.freeze(obj);
  let res = Object.isFrozen(freez);
  if(res == true) {
    return freez;
  } else {
    return "Помилка, якщо об'єкт не було заморожено";
  }
}
...
function filterObjectsByKey(arr, key) {
  ...
  if(!(Array.isArray(arr))) return [];
	...
	let res = arr.filter(element => element[key]);
  return res;
}
